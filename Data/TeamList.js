export const teams = [
    {
      image: '/genesis.JPG',
      name: 'Genesis',
      link: 'Genesis',
      bio: 'Music lover and entrepreneur turning the wildest dreams into real life experiences',
      socialLinks: {
        twitter: 'https://twitter.com/gensijay',
        instagram: 'https://instagram.com/gensijay',
      },
  position:'CEO'
    },
    {
      image: '/chris.jpeg',
      name: 'Nyinevi Christian Kekeli',
      link: 'Chris',
      bio: `He is currently the Public Relation Officer of the Education Department of the University of Ghana( Mount Mary College).
       He also the French Students  Organizer at his college. He is the road manager at Skolars Entertainment, 
       an instrumentalist (i.e. drummer and a lead guitarist) and also a Radio presenter at Rite Fm.
      He is fluent in  English, French, Ewe, Dangbe and Twi.`,
      socialLinks: {
        twitter: 'https://twitter.com/Thechriskelly3',
        instagram: 'https://instagram.com/chriskelly',
      },
  position:'Road Manager '
    },
    {
      image: '/lens.jpeg',
      name: 'Godoflenses',
      link: 'Godoflenses',
      bio: `A Film Director and Editor. Celebrated for crafting visually compelling and engaging stories. Known for his keen eye for detail and innovative approach, he transform concepts into captivating visual narratives.`,
      socialLinks: {
        twitter: 'https://twitter.com/@godoflenses',
        instagram: 'https://instagram.com/@godoflenses',
      },
  position:'Film Director'
    },
    {
      image: '/jem.jpeg',
      name: 'Jemima Mensah',
      link: 'Jemima',
      bio: ` Managing both artists and Brand's image, I also specialise in promoting activities to enhance visibility and public awareness. I am creative, Intuitive and sensitive. outside of work, I love Nature and like exploring.`,
      socialLinks: {
        twitter: 'https://twitter.com/',
        instagram: 'https://instagram.com/ama_brew_otabil',
      },
  position:'PR & Publicist'
    },
  //   {
  //     image: '/Samuel.jpeg',
  //     name: 'Samuel Boadu Ampofo',
  //     link: 'Samuel',
  //     bio: `Adventurous `,
  //     socialLinks: {
  //       twitter: 'https://twitter.com/khay curry',
  //       instagram: '',
  //     },
  // position:' Marketer'
  //   },
    {
      image: '/smoke.jpeg',
      name: 'Francis Agbeke (Smoke)',
      link: 'Smoke',
      bio: `On a journey 🚶‍♂️ | Towards success 🏆. `,
      socialLinks: {
        twitter: 'https://twitter.com/@FrancisAgbeke2',
        instagram: 'https://instagram.com/@smoke_360',
      },
  position:' Contract Admin'
    },
    {
      image: '/mel.jpeg',
      name: ' Melody Otima Ayisi ',
      link: 'Melody',
      bio: `A Baker, PR and an Artist Manager with a keen eye for talent and a heart full of passion for music. `,
      socialLinks: {
        twitter: 'https://twitter.com/@AyisiMelody',
        instagram: 'https://instagram.com/melody_ayisi',
      },
  position:' Records/Artist Manager '
    },
    {
      image: '/nathan.jpeg',
      name: 'Nathan O. Anokye',
      link: 'Nathan',
      bio: `Multi-talented artist/Pan-Africanist, Set designer & stylist  born and raised in GH, communism through art and music! `,
      socialLinks: {
        twitter: 'https://twitter.com/@Naeworldwide',
        instagram: 'https://instagram.com/Naeworldwide',
      },
  position:'Stylist/set designer'
    },
    {
      image: '/matthew.jpeg',
      name: 'Teye-Wayo Matthew',
      link: 'Matthew',
      bio: `My interest falls on finding new perspectives on different subjects, making  photography unique from other photographers, my goal is capture the beauty, strength and enthusiasm in people I focus on. `,
      socialLinks: {
        twitter: 'https://twitter.com/cyriss_Matthew',
        instagram: 'https://instagram.com/aluta_gunshot',
      },
  position:'Camera Operator'
    },




]